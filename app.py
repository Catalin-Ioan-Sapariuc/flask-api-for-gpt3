...
def get_ai_model_answer(): 
    if request.method == "POST": 
        ...
        if ( "001" in model):
            MAX_ALLOWED_TOKENS = 2040
        else:
            MAX_ALLOWED_TOKENS = 4080

        print("prompt = \n", prompt)
        #op_p = 1.
        #presence_penalty = 1.
        actual_prompt = enrich_query(prompt, max_tokens, MAX_ALLOWED_TOKENS)
        response =openai.Completion.create(model = model, prompt = actual_prompt, max_tokens=max_tokens,
                                           temperature=temperature) 
        # n=1, stop=['\n\ninput:'])
        hresponse=response['choices'][0]['text']
        print("hresponse = \n", hresponse)
